...
また、以下の表記の違いがあっても同じ地番・座標へ一致します。

- 通常住所（都道府県・市名を含む表記）
- 都道府県名・市名の省略
- 全角数字（`６７３` / `１丁目`）
- 漢数字の丁目（`一丁目`。地番の漢数字は桁の解釈が一意に決まらないため変換しません）
- 「字」の省略（`糸満市糸満673`）と「大字」表記（`糸満市大字糸満673`）
  ※ ABRの正式町字名が「字」「大字」で始まる場合だけの取り扱いです。`西崎町1丁目` のように
  接頭辞の無い町字へ `字西崎町1丁目` と書いた場合は、存在しない住所として `town_not_found`
  になります。`字上里` と `大字上里` が別々の町字として存在する場合は、正式名称どおりの入力を
  優先し、`上里` のように接頭辞を省略した入力だけ `ambiguous_town` とします
- ハイフン類（`-` `－` `ー` `‐` `―` `−` 等）
- 半角・全角スペースの混入（先頭・末尾・途中）
- 「番地」表記の省略

枝番の区切り表記（`673-2` / `673番地の2` / `673番地2` / `673番2` / `673の2`）も、同じ地番として扱います。
これらは `test/test_address_conversion.py` で回帰テストしています。

> **補足（2026-09-21）**: `673番地2` / `673番2` のように「の」を挟まない表記は、以前は
> `6732` という1つの地番として解釈されていました（同名の地番が存在すると、別の場所の座標が
> `matched` として出力される状態でした）。修正済みです。詳細は
> [docs/bugfix-2026-09-21.md](./bugfix-2026-09-21.md) を参照してください。
