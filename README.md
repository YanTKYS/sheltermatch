...
直線・ハザード区域を地図で確認できます。道路に沿った参考経路を作成した場合は、候補表の行を選ぶと
その候補の経路も表示します。**地図ライブラリ・背景地図・ハザード画像・経路の座標をすべて同梱するため、
...
- **表示**: 初期表示は従来どおり直線のみ。候補表の行を選ぶと、その候補の経路を実線で表示する（直線は破線、
  地点と道路の間の接続部分は点線）。距離は「道路上の距離（道路への接続点の間）」と接続部分の長さを分けて表示する
